...
describe GoData do
  ...
  describe 'when asked to recognize assertions' do
    ...
    it 'must accept valid testify assertions' do
      ...
      assert Class.new.extend(GoData).is_testify_assertion('Nil(t, containers.Get("invalid-id"))')
    ...
    it 'must reject invalid testify assertions' do
      assert (not Class.new.extend(GoData).is_testify_assertion('arettttt!NotEqual(t, node1.AddContainer(createContainer("c1", config)))'))
    end
