# A Mixin for the analysis of C build files. Supports CTest, which uses CTest's output

module CLogFileAnalyzer
  ...
  def extract_tests
    ...
    @folds[@OUT_OF_FOLD].content.each do |line|
      ...
      elsif !(line =~ /The command "c test/).nil? && test_section_started
        test_section_started = false
      end
