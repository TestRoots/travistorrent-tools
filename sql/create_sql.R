...
dbSendQuery(con, sprintf("CREATE INDEX part_gh_project_name ON %s (gh_project_name(60));",table.name))
dbSendQuery(con, sprintf("CREATE INDEX tr_build_id ON %s (tr_build_id);",table.name))
dbSendQuery(con, sprintf("CREATE INDEX tr_build_id ON %s (tr_prev_build_id);",table.name))
dbDisconnect(con)
